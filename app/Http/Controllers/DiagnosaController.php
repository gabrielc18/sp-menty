        ...
        foreach($data_penyakit as $final) {
            ...
            foreach($final as $key => $value) {
                ...
                if(count($final) - 1 == $key) {
                    if($cf_max == null) {
                        $cf_max = [$hasil_cf, "{$final[0]->nama} ({$final[0]->kode}) ({$final[0]->penyebab})"];
                    } else {
                        $cf_max = ($hasil_cf > $cf_max[0]) 
                            ? [$hasil_cf, "{$final[0]->nama} ({$final[0]->kode}) ({$final[0]->penyebab})"] 
                            : $cf_max;
                    }
                ...
                if(empty($hasil_diagnosa[$final[0]->id])) {
                    $hasil_diagnosa[$final[0]->id] = [
                        ...
                        'penyebab_penyakit' => $final[0]->penyebab,
                        'gejala' => [
                            [
                                'nama' => $final[$key][0]->nama,
                                'kode' => $final[$key][0]->kode,
                                'penyebab' => $final[$key][0]->penyebab,
                                'cf_user' => $final[$key][1],
                                'cf_role' => $final[$key][2],
